...
{% endcomment %}
UCP はまたロールベースアクセス制御（role-based access control; RBAC）により Docker を保護します。
...
{% endcomment %}
UCP のインスタンスをデプロイすると、開発者や IT オペレーターは Docker Engine との直接の対話は行わない形になります。
...
{% endcomment %}
UCP は Docker が提供するクラスター機能やオーケストレーション機能を活用しています。
...
> メモ
>
> OS 固有の命名として UCP コンポーネント名において、ノードのオペレーティングシステムに応じたものがあります。
...
以下の表は、マネージャーノード上にて起動する UCP サービスを示します。
